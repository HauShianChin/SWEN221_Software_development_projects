 ...
 * 
 */
public class Trick implements Serializable{//needs to be Serializable in order to be deep cloned
	...
	/**
	 * determine which is the next trump suit if player doesn't have that current suit
	 * @return
	 */
	public Card.Suit getNextTrump(){
		return null; 
	}
	...
	public void play(Player p, Card c) throws IllegalMove {
		//gets the leading card
		Card leaderCard = getCardPlayed(getLeadPlayer());
		
		//make sure it's player's turn first
		if(!p.getDirection().equals(this.getNextToPlay())){
			throw new IllegalMove("invalid move, not your turn");// throw invalid player turn 
			}
		//has to be in players hand 
		else if(!p.getHand().contains(c)){
			throw new IllegalMove("invalid move, card does not exist in your hand");// throw invalid card error
			}
		
		//the player must have a card to match the leader suit
		else if (leaderCard != null //make sure leader card exist
				&& c.suit() != leaderCard.suit() 
				&& p.getHand().matches(leaderCard.suit()).size() > 0){
			throw new IllegalMove("invalid move, you must follow the leading suit");
			}
		
		//the trump card must exist in players hand for his move to be valid 
		
	/*
		 
		if(this.getTrumps() == null) {return;}//card must exist in order for this to work 
		if(cards[0] != null ) {
			for(Card card: p.getHand()) {//iterator 
				if(!card.suit().equals(this.getTrumps())) {
					throw new IllegalMove("invalid move, you do not have a card that can trump the current one");
				}
			}
		}
			*/
