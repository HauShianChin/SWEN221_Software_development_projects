...
public class Inspector {
	...
	public Widget newWidget(String name, Rectangle dimensions) {
		...
		try {
			Constructor<?> classConstructor = widgetClass.getConstructor(dimensions.getClass()); 
			return (Widget) classConstructor.newInstance(dimensions);
		}
	...
	public Object getAttributeValue(Widget widget, String name) {
		...
		try {
			Method get = widgetClass.getDeclaredMethod(getterName);
			//needs to return a widget. 
			Object o = get.invoke(widget); 
			return o; 
		}
	...
	public void setAttribute(Widget widget, String name, Object value) {
		...
		try {
			//getting field 
			Field f = widgetClass.getDeclaredField(name);
			Method set = widgetClass.getDeclaredMethod(setterName, f.getType()); 
			set.invoke(widget, value);
